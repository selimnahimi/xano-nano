...
ConVar cl_viewbob_xscale("cl_viewbob_xscale", "1", 0, "Scale of X oscillation");
ConVar cl_viewbob_yscale("cl_viewbob_yscale", "2", 0, "Scale of Y oscillation");

ConVar cl_viewbob_xdivision("cl_viewbob_xdivision", "100", 0, "Division of X oscillation");
ConVar cl_viewbob_ydivision("cl_viewbob_ydivision", "400", 0, "Division of Y oscillation");

ConVar cl_viewbob_rotscale("cl_viewbob_rotscale", "0.1", 0, "Rotation scale (max: 1)");
...
void CGameMovement::WalkMove( void )
{
	...
	{
		float xoffset = cl_viewbob_xscale.GetFloat() * sin(1 * gpGlobals->curtime * cl_viewbob_timer.GetFloat()) * player->GetAbsVelocity().Length() * cl_viewbob_scale.GetFloat() / cl_viewbob_xdivision.GetFloat();
		float yoffset = cl_viewbob_yscale.GetFloat() * sin(2 * gpGlobals->curtime * cl_viewbob_timer.GetFloat()) * player->GetAbsVelocity().Length() * cl_viewbob_scale.GetFloat() / cl_viewbob_ydivision.GetFloat();
		float rotation = 0.0f;

		if (smove != 0.0) {
			rotation = (smove / 150) * cl_viewbob_rotscale.GetFloat();
		}
		
		player->ViewPunch(QAngle(xoffset, yoffset, rotation));
	}

	/*if (fmove > 0.0)
	{
		Vector newoffset = player->GetViewOffset();
		Vector lean, jump;
		AngleVectors(player->EyeAngles(), NULL, &lean, &jump);
		newoffset.x = lean.x * sin(gpGlobals->curtime * 10.0f) * 10.0f * (fmove / 200);
		newoffset.y = lean.y * sin(gpGlobals->curtime * 10.0f) * 10.0f * (fmove / 200);
