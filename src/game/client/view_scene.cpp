...
static void NightVision_f(void)
{
	...
	if (bDisplayed)
	{
		...
		pPlayer->EmitSound(filter, 0, "NightVision.NVOff", vOrigin); //and play sound

		DevMsg("Night Vision OFF\n");
	}
	else
	{
		...
		CLocalPlayerFilter filter;
		pPlayer->EmitSound(filter, 0, "NightVision.NVOn", vOrigin); //and play a sound

		DevMsg("Night Vision ON\n");
	}
